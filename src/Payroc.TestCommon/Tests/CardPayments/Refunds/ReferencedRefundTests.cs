...
public class ReferencedRefundTests
{
    [Test]
    public async Task CardPaymentRefunds_ReferencedRefund_Success()
    {
        ...
        var request = Data.Get<ReferencedRefund>(
            [
                ( i => i.IdempotencyKey, Guid.NewGuid().ToString() ),
                ( i => i.PaymentId, createPaymentResponse.PaymentId ),
            ]);

        var response = await client.CardPayments.Refunds.CreateReferencedRefundAsync(request);
