...
{
    [Test]
    public async Task CardPayments_Adjust_Success()
    {
        ...
        var request = Data.Get<PaymentAdjustment>(
            [
                ( i => i.IdempotencyKey, Guid.NewGuid().ToString() ),
                ( i => i.PaymentId, createPaymentResponse.PaymentId ),
            ]);

        var response = await client.CardPayments.Payments.AdjustAsync(request);
