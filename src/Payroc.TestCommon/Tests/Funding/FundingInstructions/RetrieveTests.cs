...
using Payroc.TestCommon.Factories.Funding.RequestBodies;

namespace Payroc.TestCommon.Tests.Funding.FundingInstructions;
...
public class RetrieveTests
{
    ...
    public async Task FundingInstructions_Retrieve_Success()
    {
        ...
        var retrieveFundingInstructionRequest = new RetrieveFundingInstructionsRequest
        {
            InstructionId = fundingInstructionResponse.InstructionId ?? throw new Exception("Instruction ID is null")
        };
        
        var retrieveInstructionResponse = await client.Funding.FundingInstructions.RetrieveAsync(retrieveFundingInstructionRequest);
        
        Assert.That(fundingInstructionResponse.InstructionId, Is.Not.Null);
        Assert.That(retrieveInstructionResponse.InstructionId, Is.Not.Null);
    }
}
