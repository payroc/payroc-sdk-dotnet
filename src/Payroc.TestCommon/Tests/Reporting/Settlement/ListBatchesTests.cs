...
{
    [Test]
    public async Task Settlement_ListBatches_Success()
    {
        ...
        try
        {
            var request = new ListReportingSettlementBatchesRequest { Date = testDate };
            _= await client.Reporting.Settlement.ListBatchesAsync(request);
