...
{
    [Test]
    public async Task Subscriptions_List_Success()
    {
        ...
        _ = await client.RepeatPayments.Subscriptions.CreateAsync(subscriptionRequest);
        _ = await client.RepeatPayments.Subscriptions.CreateAsync(subscriptionRequest);        
        _ = await client.RepeatPayments.Subscriptions.CreateAsync(subscriptionRequest);  
        var listRequest = new ListSubscriptionsRequest
        {
            ProcessingTerminalId = GlobalFixture.TerminalIdAvs,
            Limit = 3
        };
        
        var listResponse = await client.RepeatPayments.Subscriptions.ListAsync(listRequest);
