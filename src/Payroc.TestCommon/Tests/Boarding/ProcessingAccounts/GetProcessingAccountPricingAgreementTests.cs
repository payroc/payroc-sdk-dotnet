...
public class GetProcessingAccountPricingAgreementTests
{
    [Test]
    public async Task ProcessingAccounts_GetProcessingAccountPricingAgreement_Success()
    {
        ...
        var pricingAgreementRequest = new GetProcessingAccountPricingAgreementProcessingAccountsRequest
        {
            ProcessingAccountId = processingAccountResponse.ProcessingAccountId ?? string.Empty
        };
        
        var pricingAgreementResponse = await client.Boarding.ProcessingAccounts.GetProcessingAccountPricingAgreementAsync(pricingAgreementRequest);
        
        Assert.That(processingAccountResponse.ProcessingAccountId, Is.Not.Null);
        Assert.That(pricingAgreementResponse.IsT0, Is.True);
        Assert.That(pricingAgreementResponse.AsT0.Processor, Is.Not.Null);
    }
}
