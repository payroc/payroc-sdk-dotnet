...
public record Signature
{
    ...
    internal sealed class JsonConverter : JsonConverter<Signature>
    {
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "requestedViaDirectLink" =>
                    jsonWithoutDiscriminator.Deserialize<Payroc.SignatureByDirectLink?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize Payroc.SignatureByDirectLink"
                        ),
                "requestedViaEmail" =>
                    jsonWithoutDiscriminator.Deserialize<Payroc.SignatureByEmail?>(options)
        ...
        )
        {
            ...
            return new Signature(stringValue, stringValue);
        }
