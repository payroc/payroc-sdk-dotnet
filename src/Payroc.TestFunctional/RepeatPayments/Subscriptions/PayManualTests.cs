...
public class PayManualTests
{
    ...
    public async Task SmokeTest()
    {
        ...
        var subscriptionPaymentRequest = Data.Get<SubscriptionPaymentRequest>(
            [
                ( i => i.IdempotencyKey, Guid.NewGuid().ToString() ),
                ( i => i.ProcessingTerminalId, GlobalFixture.TerminalIdAvs ),
                ( i => i.SubscriptionId, subscriptionResponse.SubscriptionId )
                
            ]);
        
        var subscriptionPaymentResponse = await client.RepeatPayments.Subscriptions.PayAsync(subscriptionPaymentRequest);
        
        Assert.That(subscriptionPaymentResponse.SubscriptionId, Is.Not.Null);
        Assert.That(subscriptionPaymentResponse.Payment.PaymentId, Is.Not.Null);
        Assert.That(subscriptionPaymentResponse.CurrentState.Status, Is.EqualTo("active"));
        Assert.That(subscriptionPaymentResponse.CurrentState.PaidInvoices, Is.EqualTo(1));
    }
}
